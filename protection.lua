...
zpc.add_craft = function(input, output, groups)
	if minetest.get_item_group(output, "armor_use") > 0
	or minetest.get_item_group(output, "unbreakable") > 0 then
...
minetest.register_tool("encyclopedia:protection_book",{
    description = "Book of Protection",
    groups = {book=1},
    inventory_image = "encyclopedia_protection_book.png",
    wield_image = "",
